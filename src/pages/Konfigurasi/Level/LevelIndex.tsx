...
import {
    Layers, FileText, Hash, Clock,
    Plus, Edit3, Trash2, Eye, ChevronLeft, ChevronRight,
    Sparkles, Grid3X3, List, Loader2, BarChart2
} from 'lucide-react';
...
export default function LevelIndex() {
    ...
    // Fetch Level data
    const { data: levelResponse, isLoading: isLoadingLevel, refetch: refetchLevel } = useReadContract({
    ...
    // Fetch Divisi data for name mapping
    ...
    }, [isWriteSuccess, refetchLevel]);

    // Process and combine data
    const { levelList, totalItems } = useMemo(() => {
        if (!levelResponse || !divisiResponse) return { levelList: [], totalItems: 0 };

        const [rawLevel] = levelResponse as [BlockchainLevel[], bigint];
        const [rawDivisi] = divisiResponse as [BlockchainDivisi[], bigint];

        // Create Divisi Map
        const divisiMap = new Map<number, string>();
        rawDivisi.forEach(div => {
            if (!div.deleted) {
                divisiMap.set(Number(div.divisiId), div.namaDivisi);
            }
        });

        // Convert Level
        const converted: Level[] = rawLevel
            .filter(l => !l.deleted)
            .map(l => ({
                levelId: Number(l.levelId),
                divisiId: Number(l.divisiId),
                divisiName: divisiMap.get(Number(l.divisiId)) || 'Unknown Divisi',
                namaLevel: l.namaLevel,
                keterangan: l.keterangan || '-',
                createdAt: new Date(Number(l.createdAt) * 1000),
                updatedAt: new Date(Number(l.updatedAt) * 1000),
            }));

        return { levelList: converted, totalItems: converted.length };
    }, [levelResponse, divisiResponse]);

    // Pagination logic
    const totalPages = Math.ceil(totalItems / pageSize);
    const startIndex = (currentPage - 1) * pageSize;
    const endIndex = startIndex + pageSize;
    const paginatedLevel = useMemo(() =>
        levelList.slice(startIndex, endIndex),
        [levelList, startIndex, endIndex]
    );

    const isLoading = isLoadingLevel || isLoadingDivisi;
    ...
    const handleDelete = async () => {
        ...
        try {
            writeContract({
                ...
                args: [BigInt(deleteTarget.levelId)],
            });
        } catch (error) {
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            >
                                Kelola data level dan jenjang posisi
                            </motion.p>
                        </div>
                        ...
                        >
                            ...
                            {/* Add Button */}
                            <motion.button
                                onClick={() => navigate('/konfigurasi/level/create')}
                ...
                <AnimatePresence mode="wait">
                    {isLoading ? (
                        ...
                        >
                            ...
                            <p className="text-slate-500 dark:text-slate-400">Memuat data level...</p>
                        </motion.div>
                    ) : levelList.length === 0 ? (
                        <motion.div
                            key="empty"
                        ...
                        >
                            ...
                            <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">Belum Ada Data Level</h3>
                            <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-8">
                                Belum ada data level yang ditambahkan. Silakan tambahkan level baru untuk memulai.
                            </p>
                            <motion.button
                                onClick={() => navigate('/konfigurasi/level/create')}
                        ...
                        >
                            {paginatedLevel.map((level, index) => (
                                <motion.div
                                    key={level.levelId}
                                ...
                                >
                                    ...
                                    <div className="relative overflow-hidden rounded-2xl bg-white/70 dark:bg-slate-800/50 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg hover:shadow-xl transition-all duration-300">
                                        ...
                                        <div className={`relative h-28 bg-gradient-to-br ${getCardGradient(index)} overflow-hidden`}>
                                            ...
                                            >
                                                <BarChart2 className="w-6 h-6 text-white drop-shadow-lg" />
                                            </motion.div>
                                        ...
                                        <div className="p-5 relative">
                                            <div className="relative space-y-3">
                                                ...
                                                >
                                                    ...
                                                    <span>Diperbarui: {formatDate(level.updatedAt)}</span>
                                                </motion.div>
                                                ...
                                                >
                                                    ...
                                                    <motion.button
                                                        ...
                                                        onClick={() => navigate(`/konfigurasi/level/${level.levelId}/edit`)}
                ...
                {totalPages > 1 && (
                    ...
                    >
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            Menampilkan {startIndex + 1} - {Math.min(endIndex, totalItems)} dari {totalItems} Level
                        </p>
                        <div className="flex items-center gap-2">
                            <motion.button
                            ...
                            >
                                <div className="p-6 text-center">
                                    ...
                                    <p className="text-slate-600 dark:text-slate-400 mb-6">
                                        Apakah Anda yakin ingin menghapus Level <strong>{deleteTarget.namaLevel}</strong>? Tindakan ini tidak dapat dibatalkan.
                                    </p>
