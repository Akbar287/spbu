...
import { useReadContract, useWriteContract, useReadContracts } from '@/services/blockchain/wagmi';
import {
    CreditCard, Hash,
    Plus, Edit3, Trash2, ChevronLeft, ChevronRight,
    ArrowLeft, Grid3X3, List, Loader2, AlertCircle, Tag,
...
export default function StatusPurchaseIndex() {
    const navigate = useNavigate();
    const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);
    const [deleteTarget, setDeleteTarget] = useState<StatusPurchase | null>(null);
    ...
    // Generate contract calls for each id
    ...
    const contractCalls = useMemo(() => {
        ...
        for (let i = offset + 1; i <= endIndex; i++) {
            calls.push({
                address: DIAMOND_ADDRESS as `0x${string}`,
                abi: DIAMOND_ABI,
                functionName: 'getStatusPurchaseById' as const,
    ...
    const { data: listResponse, isLoading, error, refetch } = useReadContracts({
    ...
    const items = useMemo(() => {
        ...
        return listResponse
            .map((response: any) => {
                ...
                const item = response.result as BlockchainStatusPurchase;
                if (item.deleted || item.statusPurchaseId === BigInt(0)) return null;
                return {
                    id: Number(item.statusPurchaseId),
                    spbuId: Number(item.spbuId),
            ...
            .filter(Boolean) as StatusPurchase[];
    }, [listResponse]);
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            >
                                <motion.div
                                    className="p-3 bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl shadow-lg shadow-purple-500/30"
                            ...
                            >
                                Kelola status pembelian untuk proses pengadaan
                            </motion.p>
                        </div>
                        ...
                        >
                            <div className="flex items-center gap-2">
                                ...
                                <select
                                    ...
                                    className="px-3 py-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none cursor-pointer"
                                >
                            ...
                            <div className="flex items-center bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-1 shadow-sm">
                                <motion.button
                                    ...
                                    className={`p-2.5 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-purple-100 dark:bg-purple-900/50 text-purple-600 dark:text-purple-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                                ...
                                <motion.button
                                    ...
                                    className={`p-2.5 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-purple-100 dark:bg-purple-900/50 text-purple-600 dark:text-purple-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                                    whileHover={{ scale: 1.05 }}
                                    whileTap={{ scale: 0.95 }}
                                >
                            ...
                            <motion.button
                                onClick={() => navigate('/konfigurasi/status-purchase/create')}
                                className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold rounded-2xl shadow-lg shadow-purple-500/30 transition-all"
                ...
                {error && (
                    ...
                    >
                        ...
                        <motion.button
                            onClick={() => refetch()}
                            className="px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-xl"
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                        >
                            Coba Lagi
                ...
                {!isLoading && !error && items.length === 0 && (
                    ...
                    >
                        ...
                        <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-2">Belum Ada Data</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Tambahkan status purchase pertama Anda</p>
                        <motion.button
                            onClick={() => navigate('/konfigurasi/status-purchase/create')}
                            className="flex items-center gap-2 px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-xl"
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                        >
                            ...
                            >
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="relative h-28 overflow-hidden rounded-t-3xl">
                                        <motion.div
                                            ...
                                            style={{
                                                background: `linear-gradient(135deg, 
                                                    hsl(${280 + index * 15}, 70%, 55%) 0%, 
                                                    hsl(${330 + index * 15}, 70%, 50%) 100%)`,
                                            }}
                                        />
                                        ...
                                        {/* Status Badge */}
                                        <div className={`absolute top-4 left-4 px-3 py-1 backdrop-blur-md rounded-full ${item.aktif ? 'bg-green-500/30' : 'bg-red-500/30'}`}>
                                            <span className="text-xs font-bold text-white flex items-center gap-1">
                                                {item.aktif ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                                    ...
                                    <div className="p-5">
                                        ...
                                        <div className="pt-4 border-t border-slate-200/50 dark:border-slate-700/50 flex items-center gap-2">
                                            <motion.button
                                                onClick={() => navigate(`/konfigurasi/status-purchase/${item.id}`)}
                                                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-300 font-medium rounded-xl hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors"
                                            ...
                                            <motion.button
                                                className="p-2.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-300 rounded-xl hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors"
                                                onClick={() => navigate(`/konfigurasi/status-purchase/${item.id}/edit`)}
                ...
                {totalPages > 1 && (
                    ...
                    >
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            Menampilkan {startIndex} - {endIndex} dari {totalItems} Status
                        </p>
                        <div className="flex items-center gap-2">
                            <motion.button
                                        ...
                                        <React.Fragment key={page}>
                                            ...
                                            <motion.button
                                                ...
                                                className={`min-w-[40px] h-10 rounded-xl font-medium transition-all ${currentPage === page
                                                    ...
                                                    : 'bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-purple-50 dark:hover:bg-purple-900/30 hover:text-purple-600 dark:hover:text-purple-400'}`}
                                                whileHover={{ scale: 1.05 }}
                                                whileTap={{ scale: 0.95 }}
                                            >
                                                {page}
                        ...
                        >
                            <div className="p-6 text-center">
                                ...
                                <p className="text-slate-600 dark:text-slate-400 mb-6">
                                    Apakah Anda yakin ingin menghapus <strong className="text-slate-800 dark:text-white">{deleteTarget.namaStatus}</strong>? Data yang dihapus tidak dapat dikembalikan.
                                </p>
