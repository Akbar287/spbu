...
import {
    ...
    Sparkles, X, ArrowLeft, Grid3X3, List, Loader2, Fuel, AlertCircle
} from 'lucide-react';
...
export default function DivisiIndex() {
    ...
    // Fetch Divisi data
    const { data: divisiResponse, isLoading: isLoadingDivisi, refetch: refetchDivisi } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getAllDivisi',
        args: [BigInt(0), BigInt(100)], // Fetch all for now to handle client-side pagination/filtering if needed
    });
    ...
    }, [isWriteSuccess, refetchDivisi]);

    // Process and combine data
    const { divisiList, totalItems } = useMemo(() => {
        if (!divisiResponse || !spbuResponse) return { divisiList: [], totalItems: 0 };
        ...
        // Convert Divisi
        const converted: Divisi[] = rawDivisi
            .filter(d => !d.deleted)
            .map(d => ({
                divisiId: Number(d.divisiId),
        ...
        return { divisiList: converted, totalItems: converted.length };
    }, [divisiResponse, spbuResponse]);

    // Pagination logic
    const totalPages = Math.ceil(totalItems / pageSize);
    const startIndex = (currentPage - 1) * pageSize;
    const endIndex = startIndex + pageSize;
    const paginatedDivisi = useMemo(() =>
        divisiList.slice(startIndex, endIndex),
        [divisiList, startIndex, endIndex]
    );

    const isLoading = isLoadingDivisi || isLoadingSpbu;
    ...
    const handleDelete = async () => {
        ...
        try {
            writeContract({
                ...
                args: [BigInt(deleteTarget.divisiId)],
            });
        } catch (error) {
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            >
                                Kelola data divisi untuk setiap SPBU
                            </motion.p>
                        </div>
                        ...
                        >
                            ...
                            {/* Add Button */}
                            <motion.button
                                onClick={() => navigate('/konfigurasi/divisi/create')}
                ...
                >
                    {[
                        { label: 'Total Divisi', value: totalItems, icon: Layers, color: 'from-blue-500 to-indigo-600' },
                        { label: 'Total SPBU', value: spbuCount, icon: Fuel, color: 'from-emerald-500 to-teal-600' },
                        { label: 'Bulan Ini', value: 3, icon: Clock, color: 'from-amber-500 to-orange-600' }, // Dummy for now
                        { label: 'Aktif', value: totalItems, icon: Sparkles, color: 'from-pink-500 to-rose-600' },
                    ].map((stat, index) => (
                        <motion.div
                            key={stat.label}
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                            Silakan tambahkan divisi baru untuk memulai pengelolaan.
                        </p>
                    </div>
                ) : (
                    <motion.div
                    ...
                    >
                        {paginatedDivisi.map((divisi, index) => (
                            <motion.div
                                key={divisi.divisiId}
                                variants={cardVariants}
                                    ...
                                    <div className="p-5 relative">
                                        <div className="relative space-y-3">
                                            ...
                                            >
                                                ...
                                                <span>Diperbarui: {formatDate(divisi.updatedAt)}</span>
                                            </motion.div>
                                            ...
                                            >
                                                <motion.button
                                                    onClick={() => navigate(`/konfigurasi/divisi/${divisi.divisiId}`)}
                                                ...
                                                <motion.button
                                                    ...
                                                    onClick={() => navigate(`/konfigurasi/divisi/${divisi.divisiId}/edit`)}
                    ...
                    totalPages > 1 && (
                        ...
                        >
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                Menampilkan {startIndex + 1} - {Math.min(endIndex, totalItems)} dari {totalItems} Divisi
                            </p>
                            <div className="flex items-center gap-2">
                                <motion.button
                            ...
                            >
                                <div className="p-6 text-center">
                                    ...
                                    <p className="text-slate-600 dark:text-slate-400 mb-6">
                                        Apakah Anda yakin ingin menghapus <strong className="text-slate-800 dark:text-white">{deleteTarget.namaDivisi}</strong>? Tindakan ini tidak dapat dibatalkan.
                                    </p>
