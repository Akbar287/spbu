...
export default function DivisiShow() {
    ...
    // Fetch Divisi Data
    const { data: blockchainDivisi, isLoading: isLoadingDivisi, error: errorDivisi } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getDivisiById',
        args: divisiId ? [BigInt(divisiId)] : undefined,
        query: {
            enabled: !!divisiId,
        },
    });

    const spbuId = blockchainDivisi ? (blockchainDivisi as BlockchainDivisi).spbuId : undefined;
    ...
    // Format Data
    const divisiData = useMemo((): DivisiData | null => {
        if (!blockchainDivisi) return null;
        const div = blockchainDivisi as BlockchainDivisi;

        if (div.deleted || Number(div.divisiId) === 0) return null;
        ...
        return {
            divisiId: Number(div.divisiId),
            spbuId: Number(div.spbuId),
            spbuName: spbuName,
            namaDivisi: div.namaDivisi,
            keterangan: div.keterangan || '-',
            createdAt: new Date(Number(div.createdAt) * 1000),
            updatedAt: new Date(Number(div.updatedAt) * 1000),
        };
    }, [blockchainDivisi, blockchainSpbu]);

    const isLoading = isLoadingDivisi || (!!spbuId && isLoadingSpbu);
    const notFound = !isLoading && !errorDivisi && !divisiData;
    ...
    const handleDelete = async () => {
        ...
        try {
            writeContract({
                address: DIAMOND_ADDRESS as `0x${string}`,
                abi: DIAMOND_ABI,
                functionName: 'deleteDivisi',
                args: [BigInt(divisiId)],
            });
        } catch (error) {
            ...
            <div className="min-h-screen relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-center justify-center min-h-screen">
                    ...
                    >
                        ...
                        <p className="text-slate-600 dark:text-slate-400 font-medium">Memuat detail Divisi...</p>
                    </motion.div>
                </div>
            </div>
        );
            ...
            <div className="min-h-screen relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-center justify-center min-h-screen">
                    ...
                    >
                        ...
                        <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Divisi Tidak Ditemukan</h2>
                        <p className="text-slate-600 dark:text-slate-400">Data Divisi dengan ID {divisiId} tidak ditemukan.</p>
                        <motion.button
    ...
    const detailItems = [
        { label: 'ID Divisi', value: divisiData.divisiId.toString(), icon: Hash, color: 'blue' },
        { label: 'Nama Divisi', value: divisiData.namaDivisi, icon: Layers, color: 'indigo' },
        { label: 'SPBU', value: divisiData.spbuName, icon: Fuel, color: 'emerald' },
        { label: 'Keterangan', value: divisiData.keterangan, icon: FileText, color: 'violet' },
        { label: 'Dibuat', value: formatDateTime(divisiData.createdAt), icon: Clock, color: 'amber' },
        { label: 'Diperbarui', value: formatDateTime(divisiData.updatedAt), icon: Clock, color: 'cyan' },
    ];
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
                ...
                />
                <motion.div
                    className="absolute top-1/3 right-1/4 w-96 h-96 rounded-full bg-gradient-to-br from-purple-400/15 to-pink-400/15 dark:from-purple-500/20 dark:to-pink-500/20 blur-3xl"
            ...
            <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
                ...
                >
                    <div className="flex items-center justify-between flex-wrap gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            <motion.button
                                onClick={() => navigate(`/konfigurasi/divisi/${divisiData.divisiId}/edit`)}
                ...
                >
                    ...
                    <div>
                        ...
                        <p className="text-sm text-emerald-600 dark:text-emerald-400">Divisi ini sedang aktif dan beroperasi</p>
                    </div>
                </motion.div>
            </div>
                        ...
                        >
                            <div className="p-6 text-center">
                                ...
                                <p className="text-slate-600 dark:text-slate-400 mb-6">
                                    Apakah Anda yakin ingin menghapus <strong>{divisiData.namaDivisi}</strong>? Tindakan ini tidak dapat dibatalkan.
                                </p>
