...
export default function HariEdit() {
    ...
    // Fetch existing data
    const { data: blockchainHari, isLoading: isLoadingData } = useReadContract({
    ...
    useEffect(() => {
        if (hariData) {
            reset({
                namaHari: hariData.namaHari,
                hariKerja: hariData.hariKerja,
                deskripsi: hariData.deskripsi,
            });
            setIsHariKerja(hariData.hariKerja);
        }
    }, [hariData, reset]);
    ...
    const onSubmit = async (data: HariFormData) => {
        ...
        try {
            ...
            setTimeout(() => {
                navigate(`/konfigurasi/hari/${hariId}`);
            }, 1500);
        } catch (error: any) {
            console.error('Error submitting form:', error);
            setUpdateError(error.shortMessage || error.message || 'Terjadi kesalahan saat menyimpan data');
        } finally {
                ...
                <div className="relative z-10 flex flex-col items-center justify-center min-h-screen">
                    ...
                    >
                        ...
                        <p className="text-slate-500 dark:text-slate-400 max-w-md">
                            Hari dengan ID <strong>#{hariId}</strong> tidak ditemukan.
                        </p>
                        <motion.button
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
                {/* Back Button */}
                <motion.button
                    onClick={() => navigate(`/konfigurasi/hari/${hariId}`)}
                ...
                >
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="relative z-10 p-6 md:p-8 space-y-6">
                        ...
                        {updateError && (
                            <motion.div
                                className="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800 flex items-start gap-3"
