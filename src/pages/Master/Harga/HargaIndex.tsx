...
import {
    ...
    ArrowLeft, Grid3X3, List, Loader2, Star, TrendingUp, TrendingDown,
    ChartAreaIcon
} from 'lucide-react';
...
export default function HargaIndex() {
    const navigate = useNavigate();
    const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);
    const [deleteTarget, setDeleteTarget] = useState<Harga | null>(null);
    const [isDeleting, setIsDeleting] = useState(false);

    // Fetch Harga data with isDefault = true
    const { data: hargaResponse, isLoading: isLoadingHarga, refetch: refetchHarga } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getAllHargaDefaultOn',
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            >
                                Kelola harga default untuk produk BBM
                            </motion.p>
                        </div>
                        ...
                        >
                            ...
                            {/* Add Button */}
                            <motion.button
                                onClick={() => navigate('/master/harga/create')}
                            ...
                            >
                                ...
                                <span className="hidden sm:hidden lg:block">
                                    Tambah Harga
                                </span>
                            </motion.button>
                            <motion.button
                ...
                >
                    {[
                        { label: 'Total Harga Default', value: totalItems, icon: Star, color: 'from-amber-500 to-orange-600' },
                        { label: 'Total Margin', value: formatCurrency(stats.totalMargin), icon: TrendingUp, color: 'from-green-500 to-emerald-600' },
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-1">
                            Belum Ada Data Harga Default
                        </h3>
                        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                            Silakan tambahkan harga baru untuk memulai pengelolaan.
                        </p>
                    </div>
                ) : (
                    <motion.div
                            ...
                            >
                                ...
                                <div className="relative overflow-hidden rounded-2xl bg-white/70 dark:bg-slate-800/50 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg hover:shadow-xl transition-all duration-300">
                                    ...
                                    <div className={`relative h-28 bg-gradient-to-br ${getCardGradient(index)} overflow-hidden`}>
                                        ...
                                        {/* Default Badge */}
                                        <motion.div
                                            className="absolute top-3 left-3 px-3 py-1.5 bg-yellow-500/30 backdrop-blur-md rounded-full border border-yellow-300/50"
                                    ...
                                    <div className="p-5 relative">
                                        <div className="relative space-y-3">
                                            ...
                                            >
                                                <motion.button
                                                    onClick={() => navigate(`/master/harga/${harga.hargaId}`)}
                                    ...
                                    <motion.div
                                        className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-500 via-orange-500 to-yellow-500"
                ...
                {totalPages > 1 && (
                    ...
                    >
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            Menampilkan {startIndex + 1} - {Math.min(endIndex, totalItems)} dari {totalItems} Harga
                        </p>
                        <div className="flex items-center gap-2">
                            <motion.button
