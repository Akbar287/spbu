...
import {
    DollarSign, Hash, Package,
    Eye, ChevronLeft, ChevronRight,
    ArrowLeft, Grid3X3, List, Loader2, Clock, TrendingUp, TrendingDown,
    History
} from 'lucide-react';
...
const formatCurrency = (value: number): string => {
    ...
    }).format(value / 100); // Divide by 100 since value is scaled x100
};
...
export default function HargaRiwayatIndex() {
    const navigate = useNavigate();
    const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);

    // Fetch Harga data with isDefault = false (riwayat/history)
    const { data: hargaResponse, isLoading: isLoadingHarga } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getAllHargaDefaultOff',
        args: [BigInt(0), BigInt(100)],
    });

    // Fetch Produk data for name mapping
    ...
    const { hargaList, totalItems, stats } = useMemo(() => {
        if (!hargaResponse || !produkResponse) return { hargaList: [], totalItems: 0, stats: { totalMargin: 0, avgHargaJual: 0, avgHargaBeli: 0 } };

        const [rawHarga] = hargaResponse as [BlockchainHarga[], bigint];
        const [rawProduk] = produkResponse as [BlockchainProduk[], bigint];

        // Create Produk Map
        const produkMap = new Map<number, string>();
        rawProduk.forEach(produk => {
            if (!produk.deleted) {
                produkMap.set(Number(produk.produkId), produk.namaProduk);
            }
        });

        // Convert Harga
        const converted: Harga[] = rawHarga
            .filter(h => !h.deleted)
            .map(h => {
                ...
                return {
                    ...
                    produkName: produkMap.get(Number(h.produkId)) || `Produk #${h.produkId}`,
            ...
            .sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime()); // Sort by newest first

        const totalMargin = converted.reduce((acc, h) => acc + h.margin, 0);
        const avgHargaJual = converted.length > 0 ? converted.reduce((acc, h) => acc + h.hargaJual, 0) / converted.length : 0;
        const avgHargaBeli = converted.length > 0 ? converted.reduce((acc, h) => acc + h.hargaBeli, 0) / converted.length : 0;

        return { hargaList: converted, totalItems: converted.length, stats: { totalMargin, avgHargaJual, avgHargaBeli } };
    }, [hargaResponse, produkResponse]);

    // Pagination logic
    const totalPages = Math.ceil(totalItems / pageSize);
    const startIndex = (currentPage - 1) * pageSize;
    const endIndex = startIndex + pageSize;
    const paginatedHarga = useMemo(() =>
        hargaList.slice(startIndex, endIndex),
        [hargaList, startIndex, endIndex]
    );

    const isLoading = isLoadingHarga || isLoadingProduk;

    return (
        <div className="min-h-screen relative overflow-hidden">
            <div className="absolute inset-0 bg-slate-100/80 dark:bg-slate-900" />

            {/* Animated Background Gradients - Using slate/gray theme for history */}
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
                ...
                />
                <motion.div
                    className="absolute top-1/3 right-1/4 w-96 h-96 rounded-full bg-gradient-to-br from-slate-400/15 to-gray-400/15 dark:from-slate-500/20 dark:to-gray-500/20 blur-3xl"
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            >
                                Arsip harga sebelumnya yang sudah tidak aktif
                            </motion.p>
                        </div>
                        ...
                        >
                            ...
                            <div className="flex items-center bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-1 shadow-sm">
                                <motion.button
                                    ...
                                    className={`p-2.5 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-slate-100 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                                ...
                                <motion.button
                                    ...
                                    className={`p-2.5 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-slate-100 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                                    whileHover={{ scale: 1.05 }}
                                    whileTap={{ scale: 0.95 }}
                                >
                ...
                >
                    {[
                        { label: 'Total Riwayat Harga', value: totalItems, icon: History, color: 'from-slate-500 to-gray-600' },
                        { label: 'Total Margin Historis', value: formatCurrency(stats.totalMargin), icon: TrendingUp, color: 'from-green-500 to-emerald-600' },
                        { label: 'Rata-rata Harga Jual', value: formatCurrency(stats.avgHargaJual), icon: TrendingUp, color: 'from-blue-500 to-cyan-600' },
                        { label: 'Rata-rata Harga Beli', value: formatCurrency(stats.avgHargaBeli), icon: TrendingDown, color: 'from-red-500 to-orange-600' },
                    ].map((stat, index) => (
                        <motion.div
                            key={stat.label}
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                            Riwayat harga akan muncul setelah ada perubahan harga.
                        </p>
                    </div>
                ) : (
                    <motion.div
                    ...
                    >
                        {paginatedHarga.map((harga, index) => (
                            <motion.div
                                key={harga.hargaId}
                                variants={cardVariants}
                            ...
                            >
                                ...
                                <div className="relative overflow-hidden rounded-2xl bg-white/70 dark:bg-slate-800/50 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg hover:shadow-xl transition-all duration-300">
                                    {/* Card Header with Gradient - Muted colors for history */}
                                    <div className={`relative h-28 bg-gradient-to-br ${getCardGradient(index)} overflow-hidden`}>
                                        ...
                                        {/* Archived Badge */}
                                        <motion.div
                                            className="absolute top-3 left-3 px-3 py-1.5 bg-slate-500/30 backdrop-blur-md rounded-full border border-slate-300/50"
                                        ...
                                        <div className="relative space-y-3">
                                            ...
                                            >
                                                ...
                                                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                                                    Jual: {formatCurrency(harga.hargaJual)}
                                                </span>
                                            </motion.div>
                                            ...
                                            >
                                                ...
                                                <span className="text-sm text-slate-500 dark:text-slate-400">
                                                    Beli: {formatCurrency(harga.hargaBeli)} <span className="text-green-600 dark:text-green-400 font-semibold">+{formatCurrency(harga.margin)}</span>
                                                </span>
                                            </motion.div>
                                            ...
                                            >
                                                ...
                                                <span className="text-sm text-slate-500 dark:text-slate-400">
                                                    Diarsipkan: {formatDate(harga.updatedAt)}
                                                </span>
                                            </motion.div>
                                            ...
                                            >
                                                <motion.button
                                                    onClick={() => navigate(`/master/harga/riwayat/${harga.hargaId}`)}
                                                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-slate-500/10 to-gray-500/10 dark:from-slate-500/40 dark:to-gray-500/40 text-slate-600 dark:text-slate-200 font-medium rounded-xl hover:from-slate-500/20 hover:to-gray-500/20 dark:hover:from-slate-500/60 dark:hover:to-gray-500/60 transition-all border border-slate-200/50 dark:border-slate-400/50 dark:shadow-lg dark:shadow-slate-500/20 cursor-pointer"
                                    ...
                                    <motion.div
                                        className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-slate-500 via-gray-500 to-slate-500"
                ...
                {totalPages > 1 && (
                    ...
                    >
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            Menampilkan {startIndex + 1} - {Math.min(endIndex, totalItems)} dari {totalItems} Riwayat Harga
                        </p>
                        <div className="flex items-center gap-2">
                            <motion.button
                                onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                                disabled={currentPage === 1}
                                className="p-2 rounded-xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 dark:hover:bg-slate-900/30 hover:text-slate-600 dark:hover:text-slate-400 transition-all"
                                        ...
                                        <React.Fragment key={page}>
                                            ...
                                            <motion.button
                                                ...
                                                className={`min-w-[40px] h-10 rounded-xl font-medium transition-all ${currentPage === page
                                                    ...
                                                    : 'bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900/30 hover:text-slate-600 dark:hover:text-slate-400'}`}
                                                whileHover={{ scale: 1.05 }}
                                                whileTap={{ scale: 0.95 }}
                                            >
                                                {page}
...
// Card gradient colors - Muted/gray theme for history items
const getCardGradient = (index: number) => {
    const gradients = [
        'from-slate-500 via-gray-500 to-slate-600',
        'from-gray-500 via-slate-500 to-gray-600',
