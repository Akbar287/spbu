...
export default function NozzleIndex() {
    ...
    // Fetch Nozzle data
    const { data: nozzleResponse, isLoading: isLoadingNozzle, refetch: refetchNozzle } = useReadContract({
    ...
    // Fetch Dispenser Data for mapping
    ...
    }, [isWriteSuccess, refetchNozzle]);

    // Process and combine data
    const { nozzleList, totalItems, stats } = useMemo(() => {
        if (!nozzleResponse || !dispenserResponse || !produkResponse) {
            return { nozzleList: [], totalItems: 0, stats: { total: 0, active: 0, inactive: 0 } };
        }

        const rawNozzle = nozzleResponse as BlockchainNozzle[];
        const rawDispenser = dispenserResponse as BlockchainDispenser[];

        // Handle Produk Response Structure
        // Depending on implementation, getAllProduk might return [data, total] or just data
        // Based on ProdukIndex.tsx, it returns [BlockchainProduk[], bigint] or similar if paginated
        // But for safety, let's treat it carefully. If it's an array, good. If tuple, extract first.
        ...
        if (Array.isArray(produkResponse) && produkResponse.length > 0 && Array.isArray((produkResponse as any)[0])) {
            ...
            rawProduk = produkResponse as BlockchainProduk[]; // Fallback
        }

        // Create Dispenser Map
        const dispenserMap = new Map<number, string>();
        rawDispenser.forEach(d => {
            if (!d.deleted) dispenserMap.set(Number(d.dispenserId), d.namaDispenser);
        });

        // Create Produk Map
        const produkMap = new Map<number, string>();
        rawProduk.forEach(p => {
            // Assuming Produk structure has deleted field, or we check ID
            if (!p.deleted) produkMap.set(Number(p.produkId), p.namaProduk);
        });

        // Convert Nozzle
        const converted: Nozzle[] = rawNozzle
            .filter(n => !n.deleted)
            .map(n => ({
                nozzleId: Number(n.nozzleId),
                dispenserId: Number(n.dispenserId),
                dispenserName: dispenserMap.get(Number(n.dispenserId)) || 'Unknown Dispenser',
                produkId: Number(n.produkId),
                produkName: produkMap.get(Number(n.produkId)) || 'Unknown Produk',
                nama: n.namaNozzle,
                aktif: n.aktif,
                createdAt: new Date(Number(n.createdAt) * 1000),
                updatedAt: new Date(Number(n.updatedAt) * 1000),
            }));

        const total = converted.length;
        const active = converted.filter(n => n.aktif).length;
        const inactive = converted.filter(n => !n.aktif).length;

        return { nozzleList: converted, totalItems: total, stats: { total, active, inactive } };
    }, [nozzleResponse, dispenserResponse, produkResponse]);

    // Pagination logic
    const totalPages = Math.ceil(totalItems / pageSize);
    const startIndex = (currentPage - 1) * pageSize;
    const endIndex = startIndex + pageSize;
    const paginatedNozzle = useMemo(() =>
        nozzleList.slice(startIndex, endIndex),
        [nozzleList, startIndex, endIndex]
    );
    ...
    const handleDelete = async () => {
        ...
        try {
            writeContract({
                ...
                args: [BigInt(deleteTarget.nozzleId)],
            });
        } catch (error) {
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            >
                                Kelola inventaris nozzle untuk setiap dispenser
                            </motion.p>
                        </div>
                        ...
                        >
                            ...
                            {/* Add Button */}
                            <motion.button
                                onClick={() => navigate('/master/nozzle/create')}
                ...
                >
                    {[
                        { label: 'Total Nozzle', value: totalItems, icon: Gauge, color: 'from-blue-500 to-cyan-600' },
                ...
                {isLoadingNozzle ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-1">
                            Belum Ada Data Nozzle
                        </h3>
                        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                            Silakan tambahkan nozzle baru untuk memulai pengelolaan.
                        </p>
                    </div>
                ) : (
                    <motion.div
                    ...
                    >
                        {paginatedNozzle.map((nozzle, index) => (
                            <motion.div
                                key={nozzle.nozzleId}
                                variants={cardVariants}
                            ...
                            >
                                ...
                                <div className="relative overflow-hidden rounded-2xl bg-white/70 dark:bg-slate-800/50 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg hover:shadow-xl transition-all duration-300">
                                    ...
                                    <div className={`relative h-28 bg-gradient-to-br ${getCardGradient(index)} overflow-hidden`}>
                                        ...
                                        {/* Status Badge */}
                                        <motion.div
                                            className={`absolute top-3 left-3 px-3 py-1.5 backdrop-blur-md rounded-full border ${nozzle.aktif
                                        ...
                                        >
                                            <span className="text-sm font-bold text-white flex items-center gap-1.5 drop-shadow-lg">
                                                {nozzle.aktif ? <CheckCircle className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
                                                {nozzle.aktif ? 'Aktif' : 'Tidak Aktif'}
                                            </span>
                                        </motion.div>
                                        ...
                                        >
                                            <Gauge className="w-6 h-6 text-white drop-shadow-lg" />
                                        </motion.div>
                                    ...
                                    <div className="p-5 relative">
                                        <div className="relative space-y-3">
                                            ...
                                            {/* Dispenser & Produk Info */}
                                            <div className="grid grid-cols-1 gap-2">
                                                <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                                                    <Fuel className="w-4 h-4 text-blue-500" />
                                                    <span className="truncate">{nozzle.dispenserName}</span>
                                                </div>
                                                <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                                                    <Droplets className="w-4 h-4 text-purple-500" />
                                                    <span className="truncate">{nozzle.produkName}</span>
                                                </div>
                                            </div>
                                            ...
                                            >
                                                <motion.button
                                                    onClick={() => navigate(`/master/nozzle/${nozzle.nozzleId}`)}
                                                ...
                                                <motion.button
                                                    ...
                                                    onClick={() => navigate(`/master/nozzle/${nozzle.nozzleId}/edit`)}
                                                ...
                                                <motion.button
                                                    ...
                                                    onClick={() => setDeleteTarget(nozzle)}
                                                    whileHover={{ scale: 1.15, rotate: -10 }}
                                                    whileTap={{ scale: 0.9 }}
                                                >
                ...
                {totalPages > 1 && (
                    ...
                    >
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            Menampilkan {startIndex + 1} - {Math.min(endIndex, totalItems)} dari {totalItems} Nozzle
                        </p>
                        <div className="flex items-center gap-2">
                            <motion.button
