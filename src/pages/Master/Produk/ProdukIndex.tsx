...
import {
    ...
    Sparkles, ArrowLeft, Grid3X3, List, Loader2, Fuel, CheckCircle, XCircle, Gauge
} from 'lucide-react';
...
export default function ProdukIndex() {
    ...
    // Fetch Produk data
    const { data: produkResponse, isLoading: isLoadingProduk, refetch: refetchProduk } = useReadContract({
    ...
    }, [isWriteSuccess, refetchProduk]);

    // Process and combine data
    const { produkList, totalItems } = useMemo(() => {
        if (!produkResponse || !spbuResponse) return { produkList: [], totalItems: 0 };
        ...
        // Convert Produk
        const converted: Produk[] = rawProduk
            .filter(p => !p.deleted)
            .map(p => ({
                produkId: Number(p.produkId),
                spbuId: Number(p.spbuId),
                spbuName: spbuMap.get(Number(p.spbuId)) || 'Unknown SPBU',
        ...
        return { produkList: converted, totalItems: converted.length };
    }, [produkResponse, spbuResponse]);
    ...
    const paginatedProduk = useMemo(() =>
        produkList.slice(startIndex, endIndex),
        [produkList, startIndex, endIndex]
    );

    const isLoading = isLoadingProduk || isLoadingSpbu;

    // Calculate stats
    const activeCount = useMemo(() => produkList.filter(p => p.aktif).length, [produkList]);
    ...
    const handleDelete = async () => {
        ...
        try {
            writeContract({
                ...
                args: [BigInt(deleteTarget.produkId)],
            });
        } catch (error) {
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            >
                                <motion.div
                                    className="p-3 bg-gradient-to-br from-cyan-500 to-teal-600 rounded-2xl shadow-lg shadow-cyan-500/30"
                            ...
                            >
                                Kelola data produk BBM untuk setiap SPBU
                            </motion.p>
                        </div>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <select
                                    ...
                                    className="px-3 py-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none cursor-pointer"
                                >
                            ...
                            <div className="flex items-center bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-1 shadow-sm">
                                <motion.button
                                    ...
                                    className={`p-2.5 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-cyan-100 dark:bg-cyan-900/50 text-cyan-600 dark:text-cyan-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                                ...
                                <motion.button
                                    ...
                                    className={`p-2.5 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-cyan-100 dark:bg-cyan-900/50 text-cyan-600 dark:text-cyan-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                                    whileHover={{ scale: 1.05 }}
                                    whileTap={{ scale: 0.95 }}
                                >
                            ...
                            {/* Add Button */}
                            <motion.button
                                onClick={() => navigate('/master/produk/create')}
                                className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-700 hover:to-teal-700 text-white font-semibold rounded-2xl shadow-lg shadow-cyan-500/30 transition-all"
                ...
                >
                    {[
                        { label: 'Total Produk', value: totalItems, icon: Package, color: 'from-cyan-500 to-teal-600' },
                        { label: 'Total SPBU', value: spbuCount, icon: Fuel, color: 'from-emerald-500 to-green-600' },
                        { label: 'Produk Aktif', value: activeCount, icon: CheckCircle, color: 'from-green-500 to-lime-600' },
                        { label: 'Tidak Aktif', value: totalItems - activeCount, icon: XCircle, color: 'from-red-500 to-rose-600' },
                    ].map((stat) => (
                        <motion.div
                            key={stat.label}
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                            Silakan tambahkan produk baru untuk memulai pengelolaan.
                        </p>
                    </div>
                ) : (
                    <motion.div
                    ...
                    >
                        {paginatedProduk.map((produk, index) => (
                            <motion.div
                                key={produk.produkId}
                                variants={cardVariants}
                            ...
                            >
                                ...
                                <div className="relative overflow-hidden rounded-2xl bg-white/70 dark:bg-slate-800/50 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg hover:shadow-xl transition-all duration-300">
                                    ...
                                    <div className={`relative h-28 bg-gradient-to-br ${getCardGradient(index)} overflow-hidden`}>
                                        ...
                                        {/* Status Badge */}
                                        <div className={`absolute top-3 left-3 px-2.5 py-1 rounded-full flex items-center gap-1.5 ${produk.aktif ? 'bg-green-500/90' : 'bg-red-500/90'}`}>
                                            {produk.aktif ? <CheckCircle className="w-3.5 h-3.5 text-white" /> : <XCircle className="w-3.5 h-3.5 text-white" />}
                                            <span className="text-xs font-bold text-white">{produk.aktif ? 'Aktif' : 'Tidak Aktif'}</span>
                                        </div>
                                    ...
                                    <div className="p-5 relative">
                                        <div className="relative space-y-3">
                                            ...
                                            >
                                                ...
                                                <span>Diperbarui: {formatDate(produk.updatedAt)}</span>
                                            </motion.div>
                                            ...
                                            >
                                                <motion.button
                                                    onClick={() => navigate(`/master/produk/${produk.produkId}`)}
                                                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-cyan-500/10 to-teal-500/10 dark:from-cyan-500/40 dark:to-teal-500/40 text-cyan-600 dark:text-cyan-200 font-medium rounded-xl hover:from-cyan-500/20 hover:to-teal-500/20 dark:hover:from-cyan-500/60 dark:hover:to-teal-500/60 transition-all border border-cyan-200/50 dark:border-cyan-400/50 cursor-pointer"
                                                ...
                                                <motion.button
                                                    className="p-2.5 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-600/40 dark:to-orange-600/40 text-amber-600 dark:text-orange-200 rounded-xl border border-amber-200/50 dark:border-orange-400/50 hover:shadow-lg transition-all cursor-pointer"
                                                    onClick={() => navigate(`/master/produk/${produk.produkId}/edit`)}
                                                ...
                                                <motion.button
                                                    className="p-2.5 bg-gradient-to-br from-red-50 to-pink-50 dark:from-red-600/40 dark:to-pink-600/40 text-red-600 dark:text-pink-200 rounded-xl border border-red-200/50 dark:border-pink-400/50 hover:shadow-lg transition-all cursor-pointer"
                                    ...
                                    <motion.div
                                        className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-cyan-500 via-teal-500 to-emerald-500"
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                            <motion.button
                                ...
                                className="p-2 rounded-xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-cyan-50 dark:hover:bg-cyan-900/30 hover:text-cyan-600 dark:hover:text-cyan-400 transition-all"
                                        ...
                                        <React.Fragment key={page}>
                                            ...
                                            <motion.button
                                                ...
                                                className={`min-w-[40px] h-10 rounded-xl font-medium transition-all ${currentPage === page
                                                    ...
                                                    : 'bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-cyan-50 dark:hover:bg-cyan-900/30 hover:text-cyan-600 dark:hover:text-cyan-400'}`}
                                                whileHover={{ scale: 1.05 }}
                                                whileTap={{ scale: 0.95 }}
                                            >
                                                {page}
                        ...
                        >
                            <div className="p-6 text-center">
                                ...
                                <p className="text-slate-600 dark:text-slate-400 mb-6">
                                    Apakah Anda yakin ingin menghapus <strong className="text-slate-800 dark:text-white">{deleteTarget.namaProduk}</strong>? Tindakan ini tidak dapat dibatalkan.
                                </p>
