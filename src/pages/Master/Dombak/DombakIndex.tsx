...
export default function DombakIndex() {
    const navigate = useNavigate();
    const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);
    const [deleteTarget, setDeleteTarget] = useState<Dombak | null>(null);
    const [isDeleting, setIsDeleting] = useState(false);

    // Fetch Dombak data
    const { data: dombakResponse, isLoading: isLoadingDombak, refetch: refetchDombak } = useReadContract({
    ...
    }, [isWriteSuccess, refetchDombak]);

    // Process and combine data
    const { dombakList, totalItems, stats } = useMemo(() => {
        if (!dombakResponse || !spbuResponse) return { dombakList: [], totalItems: 0, stats: { total: 0, active: 0, inactive: 0 } };
        ...
        // Convert Dombak
        const converted: Dombak[] = rawDombak
            .filter(d => !d.deleted)
            .map(d => ({
        ...
        return { dombakList: converted, totalItems: total, stats: { total, active, inactive } };
    }, [dombakResponse, spbuResponse]);

    // Pagination logic
    const totalPages = Math.ceil(totalItems / pageSize);
    const startIndex = (currentPage - 1) * pageSize;
    const endIndex = startIndex + pageSize;
    const paginatedDombak = useMemo(() =>
        dombakList.slice(startIndex, endIndex),
        [dombakList, startIndex, endIndex]
    );

    const isLoading = isLoadingDombak || isLoadingSpbu;
    ...
    const handleDelete = async () => {
        ...
        try {
            writeContract({
                ...
                args: [BigInt(deleteTarget.dombakId)],
            });
        } catch (error) {
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            >
                                Kelola inventaris dombak untuk setiap SPBU
                            </motion.p>
                        </div>
                        ...
                        >
                            ...
                            {/* Add Button */}
                            <motion.button
                                onClick={() => navigate('/master/dombak/create')}
                ...
                >
                    {[
                        { label: 'Total Dombak', value: totalItems, icon: Container, color: 'from-indigo-500 to-purple-600' },
                ...
                {isLoading ? (
                ...
                ) : paginatedDombak.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        <div className="p-4 bg-slate-100 dark:bg-slate-800/50 rounded-full mb-4">
                            <Container className="w-8 h-8 text-slate-400" />
                        </div>
                        <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-1">
                            Belum Ada Data Dombak
                        </h3>
                        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                            Silakan tambahkan dombak baru untuk memulai pengelolaan.
                        </p>
                    </div>
                ) : (
                    <motion.div
                    ...
                    >
                        {paginatedDombak.map((dombak, index) => (
                            <motion.div
                                key={dombak.dombakId}
                                variants={cardVariants}
                            ...
                            >
                                ...
                                <div className="relative overflow-hidden rounded-2xl bg-white/70 dark:bg-slate-800/50 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg hover:shadow-xl transition-all duration-300">
                                    ...
                                    <div className={`relative h-28 bg-gradient-to-br ${getCardGradient(index)} overflow-hidden`}>
                                        ...
                                        {/* Status Badge */}
                                        <motion.div
                                            className={`absolute top-3 left-3 px-3 py-1.5 backdrop-blur-md rounded-full border ${dombak.aktif
                                        ...
                                        >
                                            <span className="text-sm font-bold text-white flex items-center gap-1.5 drop-shadow-lg">
                                                {dombak.aktif ? <CheckCircle className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
                                                {dombak.aktif ? 'Aktif' : 'Tidak Aktif'}
                                            </span>
                                        </motion.div>
                                        ...
                                        >
                                            <Container className="w-6 h-6 text-white drop-shadow-lg" />
                                        </motion.div>
                                    ...
                                    <div className="p-5 relative">
                                        <div className="relative space-y-3">
                                            ...
                                            >
                                                <motion.button
                                                    onClick={() => navigate(`/master/dombak/${dombak.dombakId}`)}
                                                ...
                                                <motion.button
                                                    ...
                                                    onClick={() => navigate(`/master/dombak/${dombak.dombakId}/edit`)}
                                                ...
                                                <motion.button
                                                    ...
                                                    onClick={() => setDeleteTarget(dombak)}
                                                    whileHover={{ scale: 1.15, rotate: -10 }}
                                                    whileTap={{ scale: 0.9 }}
                                                >
                ...
                {totalPages > 1 && (
                    ...
                    >
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            Menampilkan {startIndex + 1} - {Math.min(endIndex, totalItems)} dari {totalItems} Dombak
                        </p>
                        <div className="flex items-center gap-2">
                            <motion.button
