...
import {
    ...
    CheckCircle, XCircle, Umbrella, Clock, Fuel
} from 'lucide-react';
...
export default function DispenserShow() {
    ...
    const dispenserData = useMemo((): DispenserData | null => {
    ...
    // Fetch Payung Data
    const { data: blockchainPayung } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getPayungById',
        args: dispenserData ? [BigInt(dispenserData.payungId)] : undefined,
        query: {
            enabled: !!dispenserData,
        },
    });

    const payungName = useMemo(() => {
        if (!blockchainPayung) return 'Loading...';
    ...
    const isLoading = isLoadingDispenser;
    const notFound = !isLoading && !errorDispenser && !dispenserData;
            ...
            <div className="min-h-screen relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-center justify-center min-h-screen">
                    ...
                    >
                        ...
                        <p className="text-slate-600 dark:text-slate-400 font-medium">Memuat detail Dispenser...</p>
                    </motion.div>
                </div>
            </div>
        );
    ...
    const detailItems = [
        { label: 'ID Dispenser', value: dispenserData.dispenserId.toString(), icon: Hash, color: 'indigo' },
        { label: 'Nama Dispenser', value: dispenserData.namaDispenser, icon: Fuel, color: 'purple' },
        { label: 'Payung Induk', value: payungName, icon: Umbrella, color: 'blue' },
        { label: 'Dibuat', value: formatDateTime(dispenserData.createdAt), icon: Clock, color: 'slate' },
        { label: 'Diperbarui', value: formatDateTime(dispenserData.updatedAt), icon: Clock, color: 'gray' },
    ];
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
                ...
                >
                    <div className="flex items-center justify-between flex-wrap gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            <motion.button
                                onClick={() => navigate(`/master/dispenser/${dispenserData.dispenserId}/edit`)}
                ...
                {/* Status Badge */}
                <motion.div
                    className={`mb-6 p-4 backdrop-blur-sm rounded-2xl border flex items-center gap-3 ${dispenserData.aktif
                ...
                >
                    {dispenserData.aktif ? (
                        <>
                            ...
                            <div>
                                <h3 className="text-sm font-semibold text-emerald-700 dark:text-emerald-300">Dispenser Aktif</h3>
                                <p className="text-sm text-emerald-600 dark:text-emerald-400">Dispenser ini sedang aktif digunakan</p>
                            </div>
                        </>
                    ) : (
                        <>
                            ...
                            <div>
                                ...
                                <p className="text-sm text-red-600 dark:text-red-400">Dispenser ini sedang dinonaktifkan</p>
                            </div>
                        </>
                    )}
                </motion.div>
                        ...
                        >
                            <div className="p-6 text-center">
                                ...
                                <p className="text-slate-600 dark:text-slate-400 mb-6">
                                    Apakah Anda yakin ingin menghapus <strong>{dispenserData.namaDispenser}</strong>? Tindakan ini tidak dapat dibatalkan.
                                </p>
