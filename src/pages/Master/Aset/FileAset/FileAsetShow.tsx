...
export default function FileAsetShow() {
    const navigate = useNavigate();
    const { asetId, fileAsetId } = useParams<{ asetId: string; fileAsetId: string }>();
    const [showDeleteModal, setShowDeleteModal] = useState(false);
    const [isDeleting, setIsDeleting] = useState(false);

    // Fetch FileAset Data
    const { data: blockchainFileAset, isLoading, error } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getFileAsetById',
        args: fileAsetId ? [BigInt(fileAsetId)] : undefined,
        query: {
            enabled: !!fileAsetId,
        },
    });
    ...
    }, [isWriteSuccess, navigate, asetId]);

    // Process data
    const fileData = useMemo((): FileAsetDisplay | null => {
        if (!blockchainFileAset) return null;
        const f = blockchainFileAset as BlockchainFileAset;

        if (f.deleted || Number(f.fileAsetId) === 0) return null;
    ...
    const handleDelete = async () => {
        if (!fileAsetId) return;
        setIsDeleting(true);
        try {
            writeContract({
                address: DIAMOND_ADDRESS as `0x${string}`,
                abi: DIAMOND_ABI,
                functionName: 'deleteFileAset',
                args: [BigInt(fileAsetId)],
            });
            if (fileData) {
            ...
            <div className="min-h-screen relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-center justify-center min-h-screen">
                    ...
                    >
                        ...
                        <p className="text-slate-600 dark:text-slate-400">Data File dengan ID {fileAsetId} tidak ditemukan.</p>
                        <motion.button
            ...
            <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
                    ...
                    <div className="relative z-10 p-6">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <motion.a
                                ...
                                className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-amber-600 to-orange-600 text-white font-semibold rounded-2xl shadow-lg shadow-amber-500/30"
                ...
                >
                    ...
                    <div className="relative z-10 p-6 space-y-4">
                        {[
                            { label: 'ID File', value: fileData?.fileAsetId.toString(), icon: Hash, color: 'amber' },
                            { label: 'Nama Dokumen', value: fileData?.namaDokumen, icon: FileText, color: 'orange' },
                            { label: 'Nama File Asli', value: fileData?.namaFile, icon: FileImage, color: 'yellow' },
