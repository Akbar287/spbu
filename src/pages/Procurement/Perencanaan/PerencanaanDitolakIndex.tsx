...
import {
    ClipboardList, Hash, User, Package,
    Eye, ChevronLeft, ChevronRight,
    ArrowLeft, Grid3X3, List, Loader2, Droplet, XCircle, AlertCircle
} from 'lucide-react';
...
export default function PerencanaanDitolakIndex() {
    ...
    // Get total count from blockchain (only rejected items - filterStatus = 3)
    const { data: totalCountResponse } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getCountAllRencanaPembelianKonfirmasi',
        args: [3, statusPurchaseId], // filterStatus = 3 (ditolak), statusPurchaseId
    ...
    // Fetch data from blockchain with pagination (only rejected items)
    const { data: listResponse, isLoading, error, refetch } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getAllRencanaPembelianKonfirmasi',
        args: [BigInt(offset), BigInt(pageSize), statusPurchaseId, 3], // offset, limit, statusPurchaseId, filterStatus = 3 (ditolak)
        query: { enabled: statusPurchaseId !== 0n }
    });
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
                ...
                />
                <motion.div
                    className="absolute -bottom-1/2 -right-1/2 w-full h-full rounded-full bg-gradient-to-l from-pink-400/15 to-red-400/15 dark:from-pink-500/20 dark:to-red-500/20 blur-3xl"
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <motion.h1
                                className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-red-800 to-rose-600 dark:from-red-300 dark:to-rose-300 bg-clip-text text-transparent flex items-center gap-3"
                                initial={{ opacity: 0, x: -20 }}
                                animate={{ opacity: 1, x: 0 }}
                                transition={{ delay: 0.2 }}
                            >
                                <motion.div
                                    className="p-3 bg-gradient-to-br from-red-500 to-rose-600 rounded-2xl shadow-lg shadow-red-500/30"
                            ...
                            >
                                Daftar perencanaan pembelian yang ditolak
                            </motion.p>
                        </div>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <select
                                    ...
                                    className="px-3 py-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300 focus:border-red-500 focus:ring-2 focus:ring-red-500/20 outline-none cursor-pointer"
                                >
                            ...
                            <div className="flex items-center bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-1 shadow-sm">
                                <motion.button
                                    ...
                                    className={`p-2.5 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                                    whileHover={{ scale: 1.05 }}
                                    whileTap={{ scale: 0.95 }}
                                >
                                ...
                                <motion.button
                                    ...
                                    className={`p-2.5 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                                    whileHover={{ scale: 1.05 }}
                                    whileTap={{ scale: 0.95 }}
                                >
                ...
                >
                    <motion.div
                        ...
                        className="relative overflow-hidden rounded-2xl bg-white/70 dark:bg-slate-800/50 backdrop-blur-xl border border-red-200/50 dark:border-red-700/50 p-6 shadow-lg"
                        whileHover={{ scale: 1.02, y: -2 }}
                    >
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-slate-500 dark:text-slate-400">Total Ditolak</p>
                                <p className="text-3xl font-bold text-red-600 dark:text-red-400 mt-1">{totalItems}</p>
                            </div>
                ...
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center h-64">
                        ...
                        <p className="text-slate-400 dark:text-slate-500 font-medium animate-pulse">
                            Memuat data perencanaan ditolak...
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-1">
                            Tidak Ada Perencanaan Ditolak
                        </h3>
                        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                            Belum ada perencanaan pembelian yang ditolak.
                        </p>
                    </div>
                ) : (
                    <motion.div
                    ...
                    >
                        {perencanaanList.map((perencanaan, index) => (
                            ...
                            >
                                ...
                                <div className="relative overflow-hidden rounded-2xl bg-white/70 dark:bg-slate-800/50 backdrop-blur-xl border border-red-200/50 dark:border-red-700/50 shadow-lg hover:shadow-xl transition-all duration-300">
                                    ...
                                    <div className={`relative h-28 bg-gradient-to-br ${getCardGradient(index)} overflow-hidden`}>
                                        ...
                                        <motion.div
                                            className="absolute top-3 left-3 px-3 py-1.5 backdrop-blur-md rounded-full border bg-red-500/20 border-red-300/50 text-red-100"
                                    ...
                                    <div className="p-5 relative">
                                        <div className="relative space-y-3">
                                            ...
                                            >
                                                <motion.button
                                                    onClick={() => navigate(`/procurement/perencanaan/${perencanaan.id}/ditolak`)}
                                                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-red-500/10 to-rose-500/10 dark:from-red-500/40 dark:to-rose-500/40 text-red-600 dark:text-red-200 font-medium rounded-xl hover:from-red-500/20 hover:to-rose-500/20 dark:hover:from-red-500/60 dark:hover:to-rose-500/60 transition-all border border-red-200/50 dark:border-red-400/50 cursor-pointer"
                                    ...
                                    <motion.div
                                        className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-red-500 via-rose-500 to-pink-500"
                ...
                {totalPages > 1 && (
                    ...
                    >
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            Menampilkan {startIndex + 1} - {endIndex} dari {totalItems} Perencanaan Ditolak
                        </p>
                        <div className="flex items-center gap-2">
                            <motion.button
                                onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                                disabled={currentPage === 1}
                                className="p-2 rounded-xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-600 dark:hover:text-red-400 transition-all"
                                        ...
                                        <React.Fragment key={page}>
                                            ...
                                            <motion.button
                                                ...
                                                className={`min-w-[40px] h-10 rounded-xl font-medium transition-all ${currentPage === page
                                                    ...
                                                    : 'bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-600 dark:hover:text-red-400'}`}
                                                whileHover={{ scale: 1.05 }}
                                                whileTap={{ scale: 0.95 }}
                                            >
                                                {page}
...
// Card gradient colors based on index (red theme)
