...
import {
    Truck, Hash, Calendar, Package, Droplet,
    Eye, ChevronLeft, ChevronRight,
    ArrowLeft, Grid3X3, List, Loader2, AlertCircle, FileText, Car
} from 'lucide-react';
...
interface BlockchainPengirimanView {
    pengirimanId: bigint;
    tanggal: bigint;
    noDo: string;
    noPol: string;
...
export default function PengirimanIndex() {
    ...
    const { data: totalCountResponse } = useReadContract({
        ...
        abi: DIAMOND_ABI,
        functionName: 'getCounterPengiriman',
        args: [],
    });
    ...
    const { data: listResponse, isLoading, error, refetch } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getAllPengiriman',
        args: [BigInt(offset), BigInt(pageSize)],
    });

    // Convert blockchain data to display format
    const pengirimanList = useMemo((): PengirimanItem[] => {
        if (!listResponse) return [];

        const data = listResponse as BlockchainPengirimanView[];
        return data
            .filter(item => !item.deleted && Number(item.pengirimanId) !== 0)
            .map((item) => ({
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            >
                                Kelola pengiriman produk BBM dari kilang minyak
                            </motion.p>
                        </div>
                ...
                >
                    {[
                        { label: 'Total Pengiriman', value: totalItems, icon: Truck, color: 'from-blue-500 to-indigo-600' },
                        { label: 'Halaman Saat Ini', value: `${currentPage} / ${totalPages || 1}`, icon: Package, color: 'from-cyan-500 to-blue-600' },
                    ].map((stat) => (
                        <motion.div
                            key={stat.label}
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                            Konfirmasi pengajuan MS2 untuk membuat data pengiriman.
                        </p>
                    </div>
                ) : (
                    <motion.div
                                        ...
                                        <div className="relative space-y-3">
                                            ...
                                            >
                                                ...
                                                <span className="text-sm text-slate-600 dark:text-slate-300">
                                                    DO: <strong className="text-slate-800 dark:text-white">{item.noDo}</strong>
                                                </span>
                                            </motion.div>
                                                        ...
                                                        {item.produk.map((prod, pIdx) => (
                                                            ...
                                                            >
                                                                ...
                                                                <span className="text-xs font-bold text-cyan-600 dark:text-cyan-400 bg-white/50 dark:bg-slate-800/50 px-1.5 py-0.5 rounded">
                                                                    {formatNumber(prod.totalJumlah)} L
                                                                </span>
                                                            </motion.div>
                                                        ))}
                                                    </div>
                                            ...
                                            >
                                                <motion.button
                                                    onClick={() => navigate(`/procurement/pengiriman/${item.id}`)}
                ...
                {totalPages > 1 && (
                    ...
                    >
                        <motion.button
                            ...
                            className="p-3 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white dark:hover:bg-slate-800 transition-colors shadow-sm"
                                ...
                                return (
                                    <motion.button
                                        ...
                                        className={`w-10 h-10 rounded-xl font-medium transition-all ${currentPage === pageNum
                                            ...
                                            : 'bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-800'
                                            }`}
                                        whileHover={{ scale: 1.05 }}
                                        whileTap={{ scale: 0.95 }}
                                    >
                ...
                {pengirimanList.length > 0 && (
                    ...
                    >
                        Menampilkan {offset + 1} - {Math.min(offset + pengirimanList.length, totalItems)} dari {totalItems} pengiriman
                    </motion.div>
                )}
            </div>
        </div>
