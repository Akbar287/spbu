...
// Form data interface
interface LampiranFormData {
    namaDokumen: string;
}

// Validation Schema
const lampiranValidationSchema = yup.object({
...
export default function LampiranCreate() {
    ...
    const {
        ...
        handleSubmit,
        formState: { errors },
    } = useForm<LampiranFormData>({
        resolver: yupResolver(lampiranValidationSchema) as any,
        mode: 'onChange',
    });
    ...
    // Submit form
    const onSubmit = async (data: LampiranFormData) => {
        if (!selectedFile || !fileLoId) return;
        ...
        try {
            ...
            await writeContractAsync({
                ...
                abi: DIAMOND_ABI,
                functionName: 'createFileLampiranFileLo',
                args: [
                    BigInt(fileLoId),
                    cid, // IPFS CID/Hash
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="relative z-10 p-6 md:p-8 space-y-6">
                        ...
                        >
                            ...
                            <input
                                ...
                                placeholder="Contoh: Surat Jalan, Faktur Pembelian"
                        ...
                        {/* Info Box */}
                        <motion.div
                            className="p-4 bg-teal-50/80 dark:bg-teal-900/20 rounded-xl border border-teal-200/50 dark:border-teal-500/30"
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
