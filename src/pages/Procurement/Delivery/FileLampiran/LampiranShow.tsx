...
import {
    ...
    AlertCircle, Loader2, Download, Image, Package, ExternalLink
} from 'lucide-react';
...
export default function LampiranShow() {
    const navigate = useNavigate();
    const { pengirimanId, fileLoId, lampiranId } = useParams<{ pengirimanId: string; fileLoId: string; lampiranId: string }>();
    const [showDeleteModal, setShowDeleteModal] = useState(false);
    const [isDeleting, setIsDeleting] = useState(false);

    // Fetch FileLampiran Data
    const { data: blockchainFileLampiran, isLoading, error } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getFileLampiranById',
        args: lampiranId ? [BigInt(lampiranId)] : undefined,
        query: {
            enabled: !!lampiranId,
        },
    });
    ...
    useEffect(() => {
        if (isWriteSuccess) {
            setIsDeleting(false);
            navigate(`/procurement/pengiriman/${pengirimanId}/file_lo/${fileLoId}/lampiran`);
        }
    }, [isWriteSuccess, navigate, pengirimanId, fileLoId]);

    // Process data
    const fileData = useMemo((): FileLampiranDisplay | null => {
        if (!blockchainFileLampiran) return null;
        const f = blockchainFileLampiran as BlockchainFileLampiran;

        if (f.deleted || Number(f.fileLampiranFileLoId) === 0) return null;
    ...
    const handleDelete = async () => {
        if (!lampiranId) return;
        setIsDeleting(true);
        try {
            writeContract({
                address: DIAMOND_ADDRESS as `0x${string}`,
                abi: DIAMOND_ABI,
                functionName: 'deleteFileLampiranFileLo',
                args: [BigInt(lampiranId)],
            });
            if (fileData) {
            ...
            <div className="min-h-screen relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-center justify-center min-h-screen">
                    ...
                    >
                        ...
                        <p className="text-slate-600 dark:text-slate-400">Data File dengan ID {lampiranId} tidak ditemukan.</p>
                        <motion.button
                            onClick={() => navigate(`/procurement/pengiriman/${pengirimanId}/file_lo/${fileLoId}/lampiran`)}
                            className="mt-4 px-6 py-3 bg-teal-600 text-white font-semibold rounded-2xl"
            ...
            <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
                    ...
                    <div className="relative z-10 p-6">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <motion.a
                                ...
                                className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-teal-600 to-cyan-600 text-white font-semibold rounded-2xl shadow-lg shadow-teal-500/30"
                ...
                >
                    ...
                    <div className="relative z-10 p-6 space-y-4">
                        {[
                            { label: 'ID File', value: fileData?.fileLampiranId.toString(), icon: Hash, color: 'teal' },
                            { label: 'Nama Dokumen', value: fileData?.namaDokumen, icon: FileText, color: 'cyan' },
                            { label: 'Nama File Asli', value: fileData?.namaFile, icon: FileImage, color: 'emerald' },
                            { label: 'Tipe File', value: fileData?.mimeType, icon: FileText, color: 'blue' },
                            { label: 'Ukuran', value: formatFileSize(fileData?.fileSize || 0), icon: Package, color: 'green' },
