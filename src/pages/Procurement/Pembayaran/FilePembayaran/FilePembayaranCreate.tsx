...
// Form data interface
interface FilePembayaranFormData {
    namaDokumen: string;
}

// Validation Schema
const filePembayaranValidationSchema = yup.object({
...
export default function FilePembayaranCreate() {
    ...
    const {
        ...
        handleSubmit,
        formState: { errors },
    } = useForm<FilePembayaranFormData>({
        resolver: yupResolver(filePembayaranValidationSchema) as any,
        mode: 'onChange',
    });
    ...
    // Submit form
    const onSubmit = async (data: FilePembayaranFormData) => {
        if (!selectedFile || !payId) return;
        ...
        try {
            ...
            await writeContractAsync({
                ...
                abi: DIAMOND_ABI,
                functionName: 'createFilePembayaran',
                args: [
                    BigInt(payId),
                    cid, // IPFS CID/Hash
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="relative z-10 p-6 md:p-8 space-y-6">
                        ...
                        >
                            ...
                            <input
                                ...
                                placeholder="Contoh: Struk Transfer, Bukti Slip, Kwitansi"
                        ...
                        {/* Info Box */}
                        <motion.div
                            className="p-4 bg-emerald-50/80 dark:bg-emerald-900/20 rounded-xl border border-emerald-200/50 dark:border-emerald-500/30"
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            transition={{ delay: 0.4 }}
                        >
                            <p className="text-sm text-emerald-700 dark:text-emerald-300">
                                💡 File akan diupload ke IPFS dan disimpan secara permanen di blockchain.
                            </p>
                        </motion.div>
