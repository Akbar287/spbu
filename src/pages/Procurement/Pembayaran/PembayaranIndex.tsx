...
import {
    ...
    ArrowLeft, Grid3X3, List, Loader2, Droplet, CreditCard, AlertCircle, Wallet
} from 'lucide-react';
...
export default function PembayaranIndex() {
    ...
    // Fetch all status purchase to get statusPurchaseId for "Pembayaran"
    ...
    // Find statusPurchaseId for "Pembayaran" status
    const statusPurchaseId = useMemo(() => {
        ...
        const found = (statusPurchaseList as { statusPurchaseId: bigint; namaStatus: string; deleted: boolean }[])
            .find(s => s.namaStatus === 'Pembayaran' && !s.deleted);
        return found ? found.statusPurchaseId : 0n;
    }, [statusPurchaseList]);

    // Get total count from blockchain (filterStatus = 0 for all)
    const { data: totalCountResponse } = useReadContract({
        address: DIAMOND_ADDRESS as `0x${string}`,
        abi: DIAMOND_ABI,
        functionName: 'getCountAllRencanaPembelianKonfirmasi',
        args: [0, statusPurchaseId], // filterStatus = 0 (all), statusPurchaseId
    ...
    const { data: listResponse, isLoading, error, refetch } = useReadContract({
        ...
        args: [BigInt(offset), BigInt(pageSize), statusPurchaseId, 0], // offset, limit, statusPurchaseId, filterStatus = 0
        query: { enabled: statusPurchaseId !== 0n }
    });

    // Convert blockchain data to display format
    const pembayaranList = useMemo((): PembayaranItem[] => {
    ...
    const endIndex = Math.min(offset + pembayaranList.length, totalItems);
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <motion.h1
                                className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-emerald-700 to-teal-600 dark:from-emerald-300 dark:to-teal-300 bg-clip-text text-transparent flex items-center gap-3"
                                initial={{ opacity: 0, x: -20 }}
                                animate={{ opacity: 1, x: 0 }}
                            ...
                            >
                                Daftar pembelian yang siap untuk pembayaran.
                            </motion.p>
                        </div>
                ...
                >
                    ...
                    >
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-slate-500 dark:text-slate-400">Menunggu Pembayaran</p>
                                <p className="text-3xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">{totalItems}</p>
                            </div>
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                            Belum ada pembelian yang siap untuk pembayaran.
                        </p>
                    </div>
                ) : (
                    <motion.div
                    ...
                    >
                        {pembayaranList.map((pembayaran, index) => (
                            ...
                            >
                                ...
                                <div className="relative overflow-hidden rounded-2xl bg-white/70 dark:bg-slate-800/50 backdrop-blur-xl border border-emerald-200/50 dark:border-emerald-700/50 shadow-lg hover:shadow-xl transition-all duration-300">
                                    ...
                                    <div className={`relative h-28 bg-gradient-to-br ${getCardGradient(index)} overflow-hidden`}>
                                        ...
                                        <motion.div
                                            className="absolute top-3 left-3 px-3 py-1.5 backdrop-blur-md rounded-full border bg-emerald-500/20 border-emerald-300/50 text-white"
                                        ...
                                        >
                                            <h3 className="text-lg font-bold text-white drop-shadow-lg truncate">
                                                {formatTanggal(pembayaran.tanggal)}
                                            </h3>
                                        </motion.div>
                                    </div>
                                        ...
                                        <div className="relative space-y-3">
                                            ...
                                            >
                                                ...
                                                <span className="text-sm text-slate-600 dark:text-slate-300">
                                                    Diajukan oleh <strong className="text-slate-800 dark:text-white">{pembayaran.diajukanOleh}</strong>
                                                </span>
                                            </motion.div>
                                            ...
                                            >
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                                                        Produk ({pembayaran.produk.length})
                                                    </span>
                                                </div>
                                                <div className="pl-10 space-y-1.5">
                                                    {pembayaran.produk.map((produk, idx) => (
                                                        <div
                                                            key={idx}
                                            ...
                                            >
                                                <motion.button
                                                    onClick={() => navigate(`/procurement/pembayaran/${pembayaran.id}`)}
                ...
                {totalPages > 1 && (
                    ...
                    >
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            Menampilkan {startIndex + 1} - {endIndex} dari {totalItems} Pembayaran
                        </p>
                        <div className="flex items-center gap-2">
                            <motion.button
...
// Card gradient colors based on index (Emerald/Teal theme)
