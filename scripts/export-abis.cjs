...
for (const facetName of DEPLOYED_FACETS) {
    ...
    if (!fs.existsSync(artifactPath)) {
        console.log(`❌ Artifact not found: ${facetName}`);
        continue;
    }
    ...
    try {
        ...
        console.log(`✅ ${facetName}: ${functionCount} functions, ${eventCount} events`);

    } catch (error) {
        console.log(`❌ Error processing ${facetName}: ${error.message}`);
    }
}
...
console.log(`\n📦 Combined ABI: ${combinedAbi.length} items`);
console.log(`\n✅ ABIs exported to: ${OUTPUT_DIR}`);
